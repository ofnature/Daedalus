...
/// <summary>
/// Handles Warrior enmity management: coordinated tank swaps (via <see cref="TankSwapDriver"/>) plus
/// reactive emergency Provoke and proactive off-tank Shirk.
/// </summary>
public sealed class EnmityModule : IAresModule
{
    ...
    public void CollectCandidates(IAresContext context, RotationScheduler scheduler, bool isMoving)
    {
        ...
        if (target != null)
        {
            ...
            if (TankSwapDriver.TryDriveSwap(
                    ...
                    AresAbilities.Provoke, AresAbilities.Shirk))
            {
    ...
    private void TryPushEmergencyProvoke(IAresContext context, RotationScheduler scheduler,
        Dalamud.Game.ClientState.Objects.Types.IBattleNpc? target)
    {
        ...
        scheduler.PushOgcd(AresAbilities.Provoke, target.GameObjectId, priority: 1,
            onDispatched: _ =>
            {
    ...
    private void TryPushProactiveShirk(IAresContext context, RotationScheduler scheduler)
    {
        var player = context.Player;
        ...
        var target = context.TargetingService.FindEnemyForAction(
            context.Configuration.Targeting.EnemyStrategy, WARActions.HeavySwing.ActionId, player);
