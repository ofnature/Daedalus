...
- The Sage AoE-emergency behavior below now covers **WHM, AST, and SCH** too: in a party-wide emergency, White Mage's Medica/Cure III line (no more waiting on Thin Air), Astrologian's Helios line and Celestial Opposition, and Scholar's Succor and Indomitability all bypass their gates and the cross-healer reservation and outrank single-target triage. Scholar will also spend Aetherflow through its configured reserve for the emergency Indomitability (a stack is still required). Same shared toggle, all healers

### Fix — Sage: group heals are forced when the whole party is critical (1-HP mechanics)
- After abilities that drop everyone to 1 HP, recovery was serialized: the cross-healer AoE dedup let one Sage group-heal while continuously vetoing the other's every AoE heal (it re-reserved each frame), and single-target triage outranked AoE in the weave order — so the party crept back up one Druochole at a time. Now, when several party members are critically low at once (2+ below the GCD Emergency threshold), an **AoE emergency** kicks in: Ixochole, Physis II, Holos, Pneuma, and Prognosis bypass their min-target/average-HP gates and the cross-healer reservation — **both** Sages dump group heals simultaneously — and AoE recovery outranks single-target triage for weave slots. Debug shows "AoE EMERGENCY" on the firing handlers. New shared toggle: Sage settings → "Force Group Heals in Emergency" (on by default); normal chip-damage behavior is unchanged
