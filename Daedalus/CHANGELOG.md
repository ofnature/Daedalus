...
- **Never-attack** covers what no status flag can tell you: a boss that's invulnerable for *this phase only* (Alexander's Perfect Defense), a decoy, or the far half of a two-floor boss you can't actually reach — Shinryu Paradox seen from the lower floor. Those are now refused everywhere targeting resolves: current, focus, sticky and cached alike
...
### New — 69 more coffer spots, both zones
...
- **Optional: lead the nuke with Occult Cure II.** Casting the heal earns Dualcast, so the weakness-matched nuke lands instantly on the next GCD — no pausing to cast, nothing can interrupt it, and you get a 40,000-potency heal on the way. It only does this when the target's element is **already known** (no bonus to protect otherwise) and needs the **level 6** Dualcast trait. Toggle in Occult settings
- **If it primes early in a fight and then stops, the Duty tab now says why.** Almost always it's the MP floor — Cure II is 1,500 a go, so the bar drains under the limit and it goes back to hard-casting. That floor is **now a slider** (default 5,000) instead of a fixed number: drop it to keep priming longer, raise it to hold more back for raises. The other persistent reason it names is a target whose element you've never scanned
